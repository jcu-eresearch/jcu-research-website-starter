...
blocks:
  ...
  - type: "standard"
    ...
    eyebrow: "Default"
    content: |
      This is the `standard` full-width block type. Use it for ordinary explanatory content.
...
The `standard`, `feature`, and `highlight` types share the same content fields and differ only in presentation. Everything in `blocks` is optional and can be added or reordered as the project grows.
